...
function crb_init_theme() {
  ...
  wp_enqueue_script('tubePlayer', get_bloginfo('stylesheet_directory') . '/js/jQuery.tubeplayer.min.js', array('jquery'));
	wp_enqueue_script('jquery-functions', get_bloginfo('stylesheet_directory') . '/js/functions.js', array('jquery'), '1.0.3');
...
if (!function_exists('crb_setup_theme')) {
    ...
    add_image_size('inf_home_slide',                 332, 424, true);
    
    
		//add_image_size('inf_most_popular',                 256, 387, true);
    add_image_size('inf_most_popular',                 187, 352, true);
    ...
    add_image_size('inf_post_az',                      234, 441, true);
    ...
    add_image_size('inf_home_instagram_nocrop',              313, 313, false);
    //add_image_size('inf_home_inst_prod',              313, 263, true);
    add_image_size('inf_home_inst_prod_nocrop',              313, 263, false);
...
function inf_search_filter($query) {
	if(!is_admin()) {
	    if ($query->is_search) {
	        $query->set('post_type', array('post', 'rewardstyle_products', 'inf_influencer'));
	    }
	}
	return $query;
}
	...
	<div class="browse">
		<div class="shell">
			<div class="browse-holder">
				<h3><span>- </span>Browse By influencer<span> -</span></h3>
           ...
           ?>
            <li<?php echo $extraClass; ?>>
							<a href="<?php echo ($influencers_link != '#' ? add_query_arg(array('letter' => $letter), $influencers_link) : $influencers_link); ?>"><?php echo $letter; // lowercase ?></a>
							<?php
              ...
              ?>
                  ...
                  <li>
									<a href="#">
										VIEW ALL &quot;<?php echo strtoupper($letter); ?>&quot;S
									</a>
									</li>
                  <?php } ?>
								</ul>
